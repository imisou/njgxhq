<template>
    <div class="image-aside">
        <div class="g-crop-image-box">
            <div> {{cropCSS.width}}px--- {{cropCSS.height}}px</div>
            ...
            <resize-bar v-if="resize" ref="resizeBar" @resize="resizeImage" style="display:none"></resize-bar>
        </div>
    </div>
</template>
    ...
    methods: {
        ...
        setLayout(w, h) {
            let H = areaHeight,
                ...
                marginLeft = 0,
                marginTop = 0;
            console.log('setLayout areaHeight:', areaHeight, areaWidth);
            ...
            console.log('setLayout R:', width, marginLeft, height);

            this._setStyle(w, h, marginLeft, marginTop, R, true);
        },

        _setStyle(w, h, ml, mt, r, isInit) {
            console.log('set style ', w, h, ml, mt, r, this);
            ...
            if (this.isResize) {
                ...
                // }
                this.height = this.natrualHeight;
                this.width = this.natrualWidth;
        ...
        setCropBox(w, h, r) {
            console.log('setCropBox', w, h, r, this);
            if (this.isResize) {
                return;
            }
            ...
            if (this.cutHeight > imageHeight) {

                CSSObj.height = imageHeight;
            } else {
                CSSObj.height = this.cutHeight;
            }
            if (this.cutWidth > imageWidth) {
                CSSObj.width = imageWidth;
            } else {
                CSSObj.width = this.cutWidth;
            }
            ...
            //   console.log('height:'+ imageHeight , this);
            //   CSSObj.height = imageHeight < this.cutHeight ? imageHeight : this.cutHeight;
            //   // CSSObj.width = (CSSObj.height * ratioW) / ratioH;
            //   CSSObj.width = imageWidth < this.cutWidth ? imageWidth : this.cutWidth;
            ...
            console.log('crop', CSSObj.width, CSSObj.height, CSSObj.left, CSSObj.top);
            //console.log('image',imageWidth,imageHeight);
            this.cropCSS = CSSObj;
        ...
        getCropData() {
            ...
            console.log('toCropImgH',this.cropCSS,this.imgChangeRatio);
            return {
        ...
        resize(e) {
            ...
            const move = function(ev) {
                ...
                const newCropStyle = resize(ev, self.el, $container, coor, 1.9);
                
                // console.log('move',newCropStyle,newCropStyle.height.indexOf('px'));
                if( typeof newCropStyle.height === 'string'){
                    var cropHeight = newCropStyle.height;
                    if(cropHeight.indexOf('px') != -1){
                        // console.log('true')
                        var newCropHeight = cropHeight.substring(0,cropHeight.indexOf('px'));
                        // console.log(newCropHeight);
                        newCropStyle.height = isNaN(Number.parseInt(newCropHeight))?0:Number.parseInt(newCropHeight);
                    }
                }
                console.log('newCropStyle',newCropStyle);
